			...
			Heavy_Strike
				...
				verb/Heavy_Strike()
					...
					if(usr.Tension>=maxTension)
						...
						if(!usr.Secret && !usr.HasWitchCraft() || usr.Secret in  list("Goetic Virtue", "Stellar Constellation", "Elven Sanctuary") || usr.Secret == "Eldritch" && !usr.CheckSlotless("True Form") || usr.Secret == "Jagan" ||usr.Secret=="Necromancy"||usr.Secret=="Ripple"&&!usr.HasRipple()||usr.Secret=="Senjutsu"&&!usr.CheckSlotless("Senjutsu Focus") || usr.Secret =="Heavenly Restriction" && !usr.secretDatum?:hasImprovement("Heavy Strike"))//Just default Heavy Strike
