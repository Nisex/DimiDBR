/globalTracker/var/SAIYAN_DEFIANCE_THRESHOLDS = list(0.75, 1.25, 2)
...
/mob/proc/DefianceCalcs(val, mob/attcker)
    ...
    if(DefianceCounter < 10)
        if(val >= glob.SAIYAN_DEFIANCE_THRESHOLDS[1]/asc && val < glob.SAIYAN_DEFIANCE_THRESHOLDS[2] / asc)
            DefianceCounter += 1
            if(Tail)
                OMessage(10, "<font color=red>[src]'s defiance sparks!","Defiance (1) passive.")
        else if(val >= glob.SAIYAN_DEFIANCE_THRESHOLDS[2] / asc && val < glob.SAIYAN_DEFIANCE_THRESHOLDS[3] / asc)
            DefianceCounter += 2
            if(Tail)
                OMessage(10, "<font color=red>[src] grows more defiant!","Defiance (2) passive.")
        else if(val >= glob.SAIYAN_DEFIANCE_THRESHOLDS[3] / asc)
            DefianceCounter += 3
            if(Tail)
