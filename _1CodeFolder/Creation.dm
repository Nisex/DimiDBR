		...
		if(last_online)
			...
			if(src.Satiated)
				...
				if(!was_drunk)
					...
					if(src.StrTax)
						src.SubStrTax(0.25/(2 DAYS)*food_time, Forced=1)
					if(src.EndTax)
						src.SubEndTax(0.25/(2 DAYS)*food_time, Forced=1)
					if(src.SpdTax)
						src.SubSpdTax(0.25/(2 DAYS)*food_time, Forced=1)
					if(src.ForTax)
						src.SubForTax(0.25/(2 DAYS)*food_time, Forced=1)
					if(src.OffTax)
						src.SubOffTax(0.25/(2 DAYS)*food_time, Forced=1)
					if(src.DefTax)
						src.SubDefTax(0.25/(2 DAYS)*food_time, Forced=1)
					if(src.RecovTax)
						src.SubRecovTax(0.25/(2 DAYS)*food_time, Forced=1)
			if(regen_time>food_time)
				regen_time-=food_time
				if(src.StrTax)
					src.SubStrTax(0.25/(2 DAYS*regen_time))
				if(src.EndTax)
					src.SubEndTax(0.25/(2 DAYS)*regen_time)
				if(src.SpdTax)
					src.SubSpdTax(0.25/(2 DAYS)*regen_time)
				if(src.ForTax)
					src.SubForTax(0.25/(2 DAYS)*regen_time)
				if(src.OffTax)
					src.SubOffTax(0.25/(2 DAYS)*regen_time)
				if(src.DefTax)
					src.SubDefTax(0.25/(2 DAYS)*regen_time)
				if(src.RecovTax)
					src.SubRecovTax(0.25/(2 DAYS)*regen_time)
